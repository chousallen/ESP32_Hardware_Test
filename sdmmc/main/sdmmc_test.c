...
void app_main(void)
{
    ...
    // Configure GPIO0 as open drain output
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << STATUS_GPIO_PIN),
    ...
    gpio_config(&io_conf);
    gpio_set_level(STATUS_GPIO_PIN, 0);
    ...
    // All operations successful - toggle GPIO0 at 1Hz
    ESP_LOGI(TAG, "All operations successful, toggling GPIO0 at 1Hz");
    int toggle_state = 0;
    while (1) {
        gpio_set_level(STATUS_GPIO_PIN, toggle_state);
        toggle_state = !toggle_state;
        vTaskDelay(pdMS_TO_TICKS(500));  // 500ms delay for 1Hz toggle (2 toggles per second)
    }
}
